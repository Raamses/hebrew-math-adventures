...
export const ProfileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const [allProfiles, setAllProfiles] = useState<UserProfile[]>(() => {
        ...
        try {
            ...
            console.error('Failed to access localStorage for profiles:', error);
        }
    ...
    useEffect(() => {
        if (allProfiles.length > 0) {
            try {
                ...
                console.error('Failed to save profiles to localStorage (quota exceeded or security error):', error);
            }
        }
    }, [allProfiles]);
