...
export const ArcadeHUD: React.FC<ArcadeHUDProps> = ({ mode, score, lives, timeLeft, combo }) => {
    ...
    // ⚡ Bolt: Use Framer Motion's useSpring and useTransform to animate the score
    // directly in the DOM, bypassing React's render phase which eliminates 60fps re-renders.
    const scoreSpring = useSpring(score, { stiffness: 100, damping: 30, restDelta: 0.5 });
    const displayScore = useTransform(scoreSpring, (current) => Math.round(current).toLocaleString());
