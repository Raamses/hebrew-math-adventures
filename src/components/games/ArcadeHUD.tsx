...
export const ArcadeHUD: React.FC<ArcadeHUDProps> = ({ mode, score, lives, timeLeft, combo }) => {
    // ⚡ Bolt: Use framer-motion spring to lerp score directly in DOM,
    // avoiding 60fps React state re-renders of the entire HUD component.
    const animatedScore = useSpring(score, { stiffness: 100, damping: 20 });
    const displayScoreText = useTransform(animatedScore, (v) => Math.round(v).toLocaleString());
