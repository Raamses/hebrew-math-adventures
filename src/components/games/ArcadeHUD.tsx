...
export const ArcadeHUD: React.FC<ArcadeHUDProps> = ({ mode, score, lives, timeLeft, combo }) => {
    // ⚡ Bolt: Use Framer Motion's useSpring and useTransform to animate the score without triggering React re-renders at 60fps
    const animatedScore = useSpring(score, { stiffness: 50, damping: 15 });

    useEffect(() => {
        animatedScore.set(score);
    }, [score, animatedScore]);

    // Format the spring output to string
    const displayScore = useTransform(animatedScore, (v) => Math.round(v).toLocaleString());
