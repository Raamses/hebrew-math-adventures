...
export const ArcadeHUD: React.FC<ArcadeHUDProps> = ({ mode, score, lives, timeLeft, combo }) => {
    ...
    // Performance Optimization: Use framer-motion springs to bypass React render phase for score lerping
    const springScore = useSpring(score, {
        stiffness: 100,
        damping: 30,
        restDelta: 0.5
    });
    const displayScore = useTransform(springScore, (current) => Math.round(current).toLocaleString());
