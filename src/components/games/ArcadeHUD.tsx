...
export const ArcadeHUD: React.FC<ArcadeHUDProps> = ({ mode, score, lives, timeLeft, combo }) => {
    ...
    // Performance Optimization: Use framer-motion springs to bypass React render phase for frequent score updates
    const scoreSpring = useSpring(score, {
        stiffness: 100,
        damping: 30,
        restDelta: 0.5
    });

    // Transform the raw numeric spring value into a formatted, localized string
    const displayScore = useTransform(scoreSpring, (latest) => Math.round(latest).toLocaleString());
